...
//2
while (
  isNaN(writeFirstNumber) ||
  !Number.isInteger(writeFirstNumber) ||
  writeFirstNumber === ""
) {
...
//3
writeSecondNumber = Number(
  prompt("Please,write second number(more than first number)")
);

//4
while (
  isNaN(writeSecondNumber) ||
  !Number.isInteger(writeSecondNumber) ||
  writeSecondNumber === "" ||
  writeFirstNumber >= writeSecondNumber
) {
  writeSecondNumber = Number(
    prompt("Please,write integer number too (more than first number)")
  );
}
